...
const images = [
    ...
    { id: 22, src: "images/whetu3.jpg", alt: "Whetu 3", placement: "right", size: "large", caption: "Extended Whānau – Whetūrangitia, 2022. Photo: Hamish JOhns", type: "image" },
...
function preloadMedia(index, range = 3) {
    for (let i = -range; i <= range; i++) {
        const targetIndex = (index + i + images.length) % images.length;
        const image = images[targetIndex];
        
        if (image.type === 'video') {
            if (!videoCache.has(image.src)) {
                const video = document.createElement('video');
                video.src = image.src;
                video.muted = true;
                video.preload = 'metadata';
                videoCache.set(image.src, video);
            }
        } else {
...
function init() {
    ...
    preloadMedia(currentIndex, 5);
    
    // Disable koru animation to prevent conflicts
    // If you want to try to keep it, uncomment the next line
    // startKoruAnimation();
...
function showAbout() {
    ...
    // Keep the index button visible, only hide the name button
    homeBtn.classList.add('hidden');
    // indexBtn stays visible
}

function hideAbout() {
    isAboutVisible = false;
    aboutOverlay.classList.add('hidden');
    // Show the name button back
...
function setupEventListeners() {
    ...
    imageContainer.addEventListener('click', (e) => {
        // Prevent click if clicking on koru animation area
        if (!isAboutVisible) {
            nextImage();
        }
    });
